...
use StdClass,
    ...
    Artax\Negotiation\AcceptTermFactory;
...
abstract class BaseParser implements HeaderParser {
    ...
    /**
     * @var AcceptTermFactory
     */
    protected $acceptTermFactory;
    
    /**
     * @param AcceptTermFactory $acceptTermFactory
     * @return void
     */
    public function __construct(AcceptTermFactory $acceptTermFactory = null) {
        $this->acceptTermFactory = $acceptTermFactory ?: new AcceptTermFactory;
    }
    ...
    protected function getTermsFromRawHeader($rawHeader) {
        ...
        foreach (preg_split('/\s*,\s*/', $rawHeader) as $pos => $term) {
            ...
            $terms[] = $this->acceptTermFactory->make($pos, $type, $quality, $explicitQuality);
        }
